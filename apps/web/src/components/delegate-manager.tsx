...
export function DelegateManager({ accessToken }: { accessToken: string }) {
  ...
  const load = useCallback(async () => {
    try {
      const [invRes, delRes] = await Promise.all([
        fetch(`${API_BASE_URL}/client/delegate-invites`, {
          headers: { Authorization: `Bearer ${accessToken}` },
        }),
        fetch(`${API_BASE_URL}/client/delegates`, {
          headers: { Authorization: `Bearer ${accessToken}` },
        }),
      ]);
  ...
  const invite = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/client/delegate-invites`, {
        method: 'POST',
  ...
  const revoke = async (id: string) => {
    ...
    setError(null);
    try {
      await fetch(`${API_BASE_URL}/client/delegate-invites/${id}/revoke`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      load();
  ...
  return (
    <div className="mt-4 space-y-4">
      ...
      {delegates.length > 0 && (
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Delegates</p>
          <ul className="mt-1.5 space-y-1.5">
            {delegates.map((d) => (
              <li
                key={d.id}
                className="flex items-center justify-between gap-3 rounded-lg border border-[rgba(120,53,15,0.12)] bg-[rgba(255,250,240,0.82)] px-3 py-2 text-sm"
              >
                <span className="text-slate-800">
                  {delegateName(d)}
                  <span className="ml-1 text-xs text-slate-400">· {d.user?.email}</span>
            ...
            {invites.map((inv) => (
              ...
              >
                <span className="text-slate-800">
                  {inv.email}
                  <span
                    className={`ml-2 rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase ${statusTone[inv.status] || 'bg-slate-100 text-slate-500'}`}
                  >
                    {inv.status}
                  </span>
                </span>
