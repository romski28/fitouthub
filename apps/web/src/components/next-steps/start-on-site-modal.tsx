...
export function StartOnSiteModal({ isOpen, onClose }: StartOnSiteModalProps) {
  ...
  useEffect(() => {
    if (!isOpen || !isProfessional || !state.projectId || !accessToken || isPreparingOpen) return;
                ...
                <div className="mt-6 flex flex-col items-center gap-4">
                  {qrError ? (
                    ...
                    <div className="flex flex-col items-center gap-3 py-6">
                      ...
                      <p className="text-sm text-slate-300">Preparing secure QR session...</p>
                    </div>
                  ) : null}

                  {!isPreparingOpen && !qrToken && generatingQr ? (
                    <div className="flex flex-col items-center gap-3 py-6">
                      <div className="w-10 h-10 border-4 border-emerald-400 border-t-transparent rounded-full animate-spin" />
                      <p className="text-sm text-slate-300">Generating QR code...</p>
                    </div>
                  ) : !isPreparingOpen && !qrToken ? (
