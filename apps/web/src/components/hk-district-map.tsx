...
export function HkDistrictMap({
  ...
  const [fitAxis, setFitAxis] = useState<'width' | 'height'>('width');
  ...
  const mapAspectRatio = useMemo(() => {
    const parts = HK_DISTRICT_VIEWBOX.split(/\s+/).map((part) => Number.parseFloat(part));
    const width = parts[2] || 1000;
    const height = parts[3] || 733;
    return width / height;
  }, []);
  ...
  useEffect(() => {
    ...
    const recalculateFit = () => {
      ...
      setFitAxis(containerAspect > mapAspectRatio ? 'height' : 'width');
    };
