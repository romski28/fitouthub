...
type DelegateRow = {
  ...
  user: { id: string; email: string; firstName: string | null; surname: string | null; nickname: string | null } | null;
};
type Grant = {
  id: string;
...
function delegateName(d: DelegateRow): string {
  if (!d.user) return 'Delegate';
  const full = [d.user.firstName, d.user.surname].filter(Boolean).join(' ');
  return full || d.user.nickname || d.user.email || 'Delegate';
}
...
export function DelegateAccessModal({
  ...
  const [tab, setTab] = useState<'delegate' | 'email'>('delegate');
  ...
  const load = useCallback(async () => {
    try {
      const [delRes, gRes, tsRes] = await Promise.all([
        fetch(`${API_BASE_URL}/client/delegates`, { headers: { Authorization: `Bearer ${accessToken}` } }),
        fetch(`${API_BASE_URL}/projects/${projectId}/delegate-access`, { headers: { Authorization: `Bearer ${accessToken}` } }),
        fetch(`${API_BASE_URL}/projects/${projectId}/delegate-access/task-state`, { headers: { Authorization: `Bearer ${accessToken}` } }),
      ]);
      const del = delRes.ok ? await delRes.json() : [];
      const g = gRes.ok ? await gRes.json() : [];
      const ts = tsRes.ok ? await tsRes.json().catch(() => null) : null;
      setDelegates(Array.isArray(del) ? del : []);
  ...
  const grantDelegate = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/projects/${projectId}/delegate-access`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ delegateUserId: selectedDelegateId }),
      });
      if (!res.ok) {
  ...
  const revoke = async (grantId: string) => {
    ...
    setError(null);
    try {
      await fetch(`${API_BASE_URL}/projects/${projectId}/delegate-access/${grantId}/revoke`, {
        method: 'POST',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-[#D4C8A0] bg-[#F5EEDE] shadow-2xl max-h-[90vh] flex flex-col">
        <div className="shrink-0 flex items-center justify-between border-b border-[#D4C8A0] px-5 py-4">
          <h2 className="text-lg font-bold text-slate-900">Delegate access</h2>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 space-y-4">
          ...
          <div className="flex gap-2 border-b border-[#D4C8A0]">
            <button
              ...
              className={`-mb-px border-b-2 px-3 py-2 text-sm font-medium ${tab === 'delegate' ? 'border-[#b94e2d] text-[#b94e2d]' : 'border-transparent text-slate-500'}`}
            >
              My delegate
            </button>
            <button
          ...
          {tab === 'delegate' ? (
            <div className="space-y-2">
              ...
              {delegates.length === 0 && <p className="text-xs text-slate-500">No delegates yet — invite them from your profile first.</p>}
              <button type="button" disabled={busy} onClick={grantDelegate} className="w-full rounded-lg bg-[#b94e2d] px-4 py-2 text-sm font-semibold text-white hover:bg-[#a84426] disabled:opacity-50">
                Grant access
              </button>
            </div>
          ) : taskState?.active === false ? (
            <p className="rounded-lg border border-[#D4C8A0] bg-[#FDFBF3] px-3 py-2 text-xs text-slate-500">
              Site inspection is not currently available for this project. Use the delegate tab for ongoing access.
            </p>
          ) : (
            <div className="space-y-2">
              <p className="text-xs text-slate-500">
                {taskState?.phase === 'booking'
                  ? 'Send a 48h link so a delegate can book a site inspection slot.'
                  : 'Send a 48h link so a delegate can check in on site.'}
              </p>
              <input
                type="email"
                value={email}
