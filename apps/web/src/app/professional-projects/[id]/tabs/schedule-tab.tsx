...
export const ScheduleTab: React.FC<ScheduleTabProps> = ({
  ...
  return (
    <div className="rounded-3xl border border-[rgba(120,53,15,0.14)] bg-[rgba(239,231,207,0.76)] p-5 space-y-6">
      {!isInContractWorkflow ? (
        ...
        <>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-600 mt-0.5">Milestone name, start, finish and % complete.</p>
              </div>
              <span className="text-xs text-slate-600">{financialMilestones.length} financial · {workMilestones.length} non-financial</span>
            </div>

            <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[rgba(120,53,15,0.14)] bg-[rgba(245,238,219,0.75)] px-4 py-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {!scheduleNextStep && !nextStepLoading && (
                  <button
                    ...
                    className="inline-flex items-center justify-center rounded-md border border-sky-300 bg-sky-50 px-3 py-2 text-xs font-semibold text-sky-900 transition hover:bg-sky-100 disabled:opacity-50"
                  >
                    {confirmingSchedule ? 'Confirming…' : readOnly ? '✓ Accept schedule' : '✓ Confirm Schedule'}
                  </button>
                )}
              </div>
            </div>
            ...
            {scheduleNextStep && (
              <div className="flex items-start gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-emerald-900">Schedule confirmed</p>
                  {scheduleNextStep.description && (
                    <p className="mt-0.5 text-xs text-slate-700">{scheduleNextStep.description}</p>
                  )}
          ...
          {loading ? (
            <div className="rounded-3xl border border-[rgba(120,53,15,0.14)] bg-[rgba(245,238,219,0.75)] p-8 text-center">
              <p className="text-sm text-slate-700">Loading schedule...</p>
            </div>
            ...
            <div className="rounded-3xl border border-[rgba(120,53,15,0.14)] bg-[rgba(239,231,207,0.76)] p-8 text-center">
              <p className="text-sm text-slate-700 mb-4">📋 No milestones set up yet.</p>
              {!readOnly && (
                <button
                  onClick={() => setIsAddingNew(true)}
            ...
            <div className="overflow-x-auto rounded-2xl border border-[rgba(120,53,15,0.14)] bg-[rgba(245,238,219,0.5)]">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[rgba(120,53,15,0.1)] text-slate-700">
                  ...
                  <tbody>
                    {combinedMilestones.map((milestone, index) => {
                      ...
                      return (
                        <tr key={milestone.id || index} className="border-b border-[rgba(120,53,15,0.08)]">
                          ...
                          <td className="px-3 py-3">
                            {readOnly ? (
                              ...
                              <input
                                ...
                                className="w-20 rounded-md border border-[rgba(120,53,15,0.2)] bg-white px-2 py-1 text-xs text-slate-900"
                              />
      ...
      {deleteConfirmIndex !== null && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-[rgba(239,231,207,0.9)] border border-[rgba(120,53,15,0.2)] rounded-lg p-6 max-w-sm shadow-lg">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-4 py-2 text-sm font-medium text-slate-900 bg-slate-200 rounded hover:bg-slate-300 transition"
              >
                Cancel
              </button>
              <button
